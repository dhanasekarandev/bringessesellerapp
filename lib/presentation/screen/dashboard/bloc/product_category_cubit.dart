...
class ProductCategoryCubit extends Cubit<MenuCategoryState> {
  AuthRepository authRepository;

  ProductCategoryCubit({required this.authRepository})
      : super(MenuCategoryState.initial());

  void login(categoryReqId) async {
    emit(state.copyWith(networkStatusEnum: NetworkStatusEnum.loading));

    (bool, dynamic) response = await authRepository.getproductCat(categoryReqId);
